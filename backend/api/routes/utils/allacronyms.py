...
SITE = {
    ...
    ,'random'       :   '/random/abbreviated?nocache=1'
}
...
class AllAcronyms():
    ...
    def _extractAbbreviations( self, SearchResultXML, CalculateConfidence=True ):
        ...
        '''
        print('here')
        AbbrevElms = SearchResultXML.xpath('//div[@class="terms items_text"]')   # 1 is the actual results, 2 is RELATED
        print('abbrev')
        print(AbbrevElms)
        Abbreviations = []
    ...
    def _search( self, Keywords, Category=None, Topic=None, TopCount=1 ):
        ...
        SearchResponse = requests.get( SearchURL, headers=headers )
    ...
    def getRandom( self ):
        ...
        print('url ', RandomURL)
        print('Response: ',   RandomResponse.text )
        print('Result: ',  RandomResultXML)
        return self._extractAbbreviations( RandomResultXML )
